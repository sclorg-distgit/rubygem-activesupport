...
Requires: %{?scl_prefix}rubygem(i18n) >= 0.6.4
#Requires: %{?scl_prefix}rubygem(i18n) < 1.0
Requires: %{?scl_prefix_ruby}rubygem(minitest) >= 4.2
Requires: %{?scl_prefix_ruby}rubygem(minitest) < 5
...
Requires: %{?scl_prefix}rubygem(tzinfo) >= 0.3.37
Requires: %{?scl_prefix}rubygem(tzinfo) < 0.4.0
...
BuildRequires: %{?scl_prefix}rubygem(i18n) >= 0.6.4
...
BuildRequires: %{?scl_prefix}rubygem(tzinfo) >= 0.3.37
BuildRequires: %{?scl_prefix}rubygem(tzinfo) < 0.4.0
BuildArch: noarch
...
%{?scl:scl enable %scl - << \EOF}
# Newer builder installed (xml errors); old version of minitest (assert_equal madness, 4.7 fixes the issue)
